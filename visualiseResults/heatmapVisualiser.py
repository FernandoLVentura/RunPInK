...
class MAPVisualizer():
    ...
    def readMap(self):
        ...
        try:
            while True:
                data = numpy.ones( (self.__neuronWidth * self.__neuronHeight, 4) )
                for i in range(self.__neuronWidth * self.__neuronHeight):
                    data[i, 0] = struct.unpack_from("f", inputStream.read(4))[0]
                    data[i, 1] = struct.unpack_from("f", inputStream.read(4))[0]
                    data[i, 2] = struct.unpack_from("f", inputStream.read(4))[0]
                    data[i, 3] = struct.unpack_from("f", inputStream.read(4))[0]
                    ...
                    for yPos in range(self.__neuronHeight):
                        if math.floor(xPos/2.0 + yPos) < self.__neuronHeight / 4.0 or \
                           ...
                           math.floor(xPos/2.0 - yPos) < -self.__neuronWidth + self.__neuronHeight / 4.0:
                            ...
                            image[y][x] = neuron[xPos + yPos * self.__neuronWidth]
            return image

    def showMap(self):
        figure = pyplot.figure("SOM", figsize=(16, 16))
        figure.patch.set_alpha(0.0)
        figure.patch.set_facecolor('#333333') 
